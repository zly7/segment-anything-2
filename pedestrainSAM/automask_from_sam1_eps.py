...
class PedestrainSamAutomaticMaskGenerator:
    def __init__(
        ...
        self.max_prompt_times = 30
        if self.mask_selection == 'max_iou_k' and self.max_iou_k == -1:
            raise ValueError("max_iou_k requires max_iou_k to be set.")
    ...
    def _process_crop(
        ...
        data = MaskData()
        img_size = torch.tensor(cropped_im.shape[:2])
        occupy_mask = torch.zeros(*img_size, dtype=torch.bool, device=self.device)

        def efficient_batch_iterator(batch_size: int, points: np.ndarray):
            np.random.shuffle(points)
            count = 0
            while len(points) > 0 and count < self.max_prompt_times:
                current_batch_size = min(len(points), batch_size)
                sel_pts = points[:current_batch_size]
                points = points[current_batch_size:] 
                yield sel_pts
                # 更新 occupy_mask 以过滤已经被覆盖的点
                keep = ~occupy_mask[points[:, 1], points[:, 0]].cpu().numpy() # 这句话关心points的位置现在是不是occupy_mask
                points = points[keep]
                count += 1
        # Iterate over batches of points using efficient_batch_iterator
        for sel_pts in efficient_batch_iterator(self.points_per_batch, points_for_image):
            if len(sel_pts) == 0:
                continue
            # Prepare batch-specific data
            batch_points = torch.as_tensor(sel_pts[:, None, :], device=self.predictor.device)
            batch_labels = torch.ones(batch_points.shape[:2], dtype=torch.int, device=self.predictor.device)
            batch_transformed_points = self.predictor._transforms.transform_coords(
                batch_points, normalize=True, orig_hw=cropped_im_size
            )
            batch_sparse_embeddings, batch_dense_embeddings = self.predictor.sam2_model.sam_prompt_encoder(
                points=(batch_transformed_points, batch_labels),
                boxes=None,
                masks=None,
            )

            # Call _process_batch with all required parameters
            ...
            # Update occupy_mask correctly by iterating over masks and iou_preds
            if len(batch_data["masks"]) > 0:
                for mask, iou_pred in zip(batch_data['masks'], batch_data['iou_preds']):
                    occupy_mask |= mask.any(0)
    ...
    def _process_batch(
        ...
        def select_mask(self, masks, iou_preds):
            ...
            if self.mask_selection == 'max_iou':
                max_iou_index = iou_preds.argmax()
                keep_mask = torch.zeros(num_masks, dtype=torch.bool, device=masks.device)
                keep_mask[max_iou_index] = True
            elif self.mask_selection == 'max_iou_k':
                topk = self.max_iou_k
                if topk > num_masks:
                    return torch.ones(num_masks, dtype=torch.bool, device=masks.device)
                topk_values, topk_indices = iou_preds.topk(topk)
                keep_mask = torch.zeros(num_masks, dtype=torch.bool, device=masks.device)
                keep_mask[topk_indices] = True
            elif self.mask_selection == 'all':
                keep_mask = torch.ones(num_masks, dtype=torch.bool, device=masks.device)
            else:
                raise NotImplementedError(f"mask_selection '{self.mask_selection}' 未实现。")
            return keep_mask
        final_indices = select_mask(self, data["masks"], data["iou_preds"])
        data.filter(final_indices)
